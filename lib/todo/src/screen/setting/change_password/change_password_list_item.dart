  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      ...
      builder: (context, AsyncSnapshot<PasswordListResponseModel> snapshot) {
            return Padding(
              padding: (data.isExpand == true)
                  ? const EdgeInsets.all(12.0)
                  : const EdgeInsets.all(12.0),
              child: Container(
                decoration: BoxDecoration(
                  color: Colors.white,
                  borderRadius: (data.isExpand != true)
                      ? BorderRadius.all(
                          Radius.circular(10),
                        )
                      : BorderRadius.all(
                ...
                child: ExpansionTile(
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        Text(
                          data.type,
                          style: TextStyle(
                              fontSize: (data.isExpand != true) ? 18 : 18, color: Colors.black),
                        ),
                        Text('This Week', textAlign: TextAlign.end, style: TextStyle(fontSize: 12),)
                      ],
                    )
                  ),
                  trailing: (data.isExpand == true)
                      ? Icon(
                  ...
                  onExpansionChanged: (value) {
                    print('old');
                    print(data.isExpand);
                    bloc.updatePasswordIsExpand(data.id);
                  },
                  children: [
